...
class Trainer:
    ...
    def run_fit_blend(self, total_iters = 500):
        # build bar
        fit_bar = tqdm(range(1, total_iters+1)) if self._debug else                 rtqdm(range(1, total_iters+1))
        train_iter = iter(self.train_dataloader)
        blend = self.model.blendmodel
        blend.train()

        params = [{'params': [blend.exp_face_displace], 'lr': 0.001}]
        optimizer_blend = torch.optim.Adam(params)
        
        print("Run blend fitting")
        loss_func = nn.functional.l1_loss

        for iter_idx in fit_bar:
        #for iter_idx in range(1, self._total_iters+1):
            ...
            t_points = batch_data['t_points']
            t_blend = batch_data['t_blend']
            t_transform = batch_data['t_transform']

            pred_points = blend(t_blend)

            loss_offset = blend.exp_face_displace.abs().mean()
            blend_loss = loss_func(pred_points, t_points)

            loss = blend_loss + loss_offset 
            #blend_loss.backward()
            self.lightning_fabric.backward(loss)
            optimizer_blend.step()
            optimizer_blend.zero_grad(set_to_none=True)
            
            #print("Mse loss: ", blend_loss.item())
            fit_bar.set_description((f"L1: {blend_loss.item()} L1 offset:{loss_offset.item()}"))

            if iter_idx % 100 == 0:
                flame_mesh = blend.render_shape(t_points, t_transform, focal_length = self.model.cam_params["focal_x"]).detach()
                blend_mesh = blend.render_shape(pred_points, t_transform, focal_length = self.model.cam_params["focal_x"]).detach()
                cat = torch.cat((flame_mesh[0], blend_mesh[0]), dim=2)
                torchvision.utils.save_image(cat,  os.path.join(f'./temp/cat_flame_blend_{iter_idx}.png'))

            loss = blend_loss + loss_offset 
        print("Fitting Blend Finished! Loss L1: ", blend_loss.item())

        test_iter = iter(self.val_dataloader)
        for i in range(10):
            try:
            ...
            cat = torch.cat((flame_mesh[0], blend_mesh[0]), dim=2)
            torchvision.utils.save_image(cat,  os.path.join(f'./temp/val_flame_blend_{i}.png'))

            loss = blend_loss #+ loss_offset 
            print(loss)
        
        self.model.blendmodel.exp_face_displace.requires_grad = False

    def run_fit(self, ):
        
        self.run_fit_blend(total_iters=1000)
