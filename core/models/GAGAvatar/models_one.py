...
class GAGAvatar_spade32(nn.Module):
    def __init__(self, model_cfg=None, **kwargs):
        ...
        self.upsampler = Unet_rgb(in_size=512, in_dim=32, out_dim=3, out_size=512)

        self.percep_loss = FacePerceptualLoss(loss_type='l1', weighted=True)

        self.blendmodel = BlendModel('blendmodel')
        ...
        #for param in self.blendmodel.parameters():
        #    param.requires_grad = True

        self.blendmodel.exp_face_displace.requires_grad = True
        self.blendmodel.exp_face.requires_grad = False

        times = 5
        batch_size = 1
        gs_params ={}
        self.gs_params_xyz = nn.Parameter(torch.randn(batch_size, 5023 * times * 2, 3), require_grad = True)
        self.gs_params_colors = nn.Parameter(torch.randn(batch_size, 5023 * times * 2, 32), require_grad = True)
        self.gs_params_scales = nn.Parameter(torch.randn(batch_size, 5023 * times * 2, 3), require_grad = True)
        self.gs_params_opacites = nn.Parameter(torch.randn(batch_size, 5023 * times * 2, 1), require_grad = True)
        self.gs_params_rotations = nn.Parameter(torch.randn(batch_size, 5023 * times * 2, 4), require_grad = True)

        self.zeros = torch.zeros((batch_size, 5023 * times * 2, 3)).to("cuda")


    def forward(self, batch, train_frac=1.0, rand=True, mesh = False):
        ...
        #     ), plane_direnc
        # )
        # # colors, opacities, scale, rotations
        # #gs_params_g['xyz'] = t_points #(5023, 3)
        # gs_params_g['xyz'] = pred_points
        times = 10
        ...
        # #image_mesh = self.blendmodel.render(pred_points, t_transform, focal_length = self.cam_params["focal_x"])
        #flame_mesh = self.blendmodel.render(t_points, t_transform, focal_length = self.cam_params["focal_x"])
        if mesh:
            flame_mesh = self.blendmodel.render_shape(t_points, t_transform, focal_length = self.cam_params["focal_x"]).detach()
            blend_mesh = self.blendmodel.render_shape(pred_points, t_transform, focal_length = self.cam_params["focal_x"]).detach()
        
        # save_image(image_mesh[0],  os.path.join(f'./temp/blend_mesh_0.png'))
        # save_image(t_image[0],  os.path.join(f'./temp/image_0.png'))
        # save_image(flame_mesh[0],  os.path.join(f'./temp/flame_0.png'))
        ...
        if mesh:
            results['blend_mesh'] = blend_mesh
            results['flame_mesh'] = flame_mesh
        return results

    @torch.no_grad()
    def forward_expression(self, batch, mesh = True):
        if not hasattr(self, '_gs_params'):
            batch_size = batch['f_image'].shape[0]
            f_image, f_planes = batch['f_image'], batch['f_planes']
            f_feature0, f_feature1 = self.base_model(f_image) #(1, 256, 296, 296) 
            # dir encoding
            plane_direnc = self.harmo_encoder(f_planes['plane_dirs'])        #(1, 3) , plane_points:87616
            # global part
            gs_params_g = self.gs_generator_g(
                torch.cat([
                        self.head_base[None].expand(batch_size, -1, -1), f_feature1[:, None].expand(-1, 5023, -1), 
        ...
        # from IPython import embed 
        # embed()
        # color, opacities, scales, rotations, xyz
    ...
    def calc_metrics(self, results):
        ...
        #point_loss = square_distance(results['p_points'], results['t_points']).mean()
        #point_loss = 0

        blend_loss = nn.MSELoss()(results['pre_points'], results['t_points']) * 10

        loss = {'percep_loss': pec_loss, 'img_loss': img_loss, 'box_loss': box_loss, "blend_point": blend_loss} #, 'point_loss': point_loss}
        # print(loss)
        psnr = -10.0 * torch.log10(nn.functional.mse_loss(t_image, sr_gen_image).detach())

        return loss, {'psnr':psnr.item(), 'Blend_point': blend_loss.item()}
    ...
    def configure_optimizers(self, config):
        ...
        print('optimize params: {}'.format(set(optimize_names)))
